...
(async () => {
  ...
  await page.waitForSelector("select[name='SelectTheme']", { visible: true });
  let theme = await page.$("select[name='SelectTheme']");
  await theme.type(
    "Promotion and popularization of Shree Anna/ Millets for nutritional wellbeing"
  );

  await page.waitForSelector("select[name='SelectLevel']", { visible: true });
  ...
  let awc_center = await page.$("select[name='awc_center']", { visible: true });
  ...
  await page.evaluate((el) => {
    ...
    // dateInput.setAttribute("value", el);
  ...
  await dateFrom.type(_date, { delay: 500 });
  await dateTo.type(_date, { delay: 500 });

  // await page.setJavaScriptEnabled(false);

  // const dateInput = await page.$("input[name='SelectDateFrom']");
  // await dateInput.type("2023-05-01");
  // await page.evaluate(() => {
  //   const dateInput = document.querySelector("input[name='SelectDateFrom']");
  //   const event = new Event("change", { bubbles: true });
  //   dateInput.dispatchEvent(event);
  // });

  // await page.setJavaScriptEnabled(true);
  // console.log(dateInput.$eval(dateInput,el => el.getAttribute("value")))
  // await page.setJavaScriptEnabled(false);

  // const dateInput1 = await page.$("input[name='SelectDateTo']");
  // await dateInput1.type("2023-05-01");
  // await page.evaluate(() => {
  //   const dateInput1 = document.querySelector("input[name='SelectDateTo']");
  //   const event = new Event("change", { bubbles: true });
  //   dateInput1.dispatchEvent(event);
  // });

  // await page.setJavaScriptEnabled(true);

  // await page.focus('input[name="SelectDateFrom"]');
  // await page.keyboard.type("29042023");

  await page.type("input[name='CountAdultMale']", randomInt(7, 14).toString());
  await page.type(
    "input[name='CountAdultFemale']",
    randomInt(7, 14).toString()
  );
  await page.type("input[name='CountChildMale']", randomInt(7, 14).toString());
  await page.type(
